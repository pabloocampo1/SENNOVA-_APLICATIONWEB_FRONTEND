...
const ChangeEmailCompo = ({ authObject = {}, setAuthObject, onClose }) => {
    ...
    const handleSumit = (e) => {
        ...
        const fetchCode = async () => {
            try {
                ...
                const res = await api.post(`/auth/generateToken/change/email/${authObject.email}/${newEmail}`);
                if (res.data) {
                    setError(false)
                    setMessageError("")
                ...
                if (error.response.data.status == 404) {
                    ...
                    msg = msg.replace(/^java\.lang\.IllegalArgumentException:\s*/, "");
                    setMessageError(msg)
                }
            } finally {
                setIsLoanding(false); 
    ...
    const handleSumitCode = (e) => {
        ...
        const fetchCode = async () => {
            try {
                ...
                const res = await api.post(`/auth/validate/code/changeEmail/${code}`);
                if (res.status == 200) {
                    ...
                    if (res.data) {
                        ...
                        setError(true)
                        setMessageError("Ocurrio un error intentado cambiar tu email")
    ...
    if (tokenCorrect) {
        return (
            <Box sx={{ width: "400px", height: "400px", display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center" }}>
                <Typography sx={{ textAlign: "center", pb: "20px" }}>Email cambiado exitosamente</Typography>
                <img src={imageSucess} alt="image success" width={200} />

                <Button sx={{ mt: "20px" }} variant='contained' onClick={() => onClose()}>Cerrar</Button>
            </Box>
        )
    }
    ...
    return (
        <Box sx={{ width: "400px", height: "200px", display: "flex", flexDirection: "column", justifyContent: "center" }}>
            ...
            {error && (<Typography sx={{ pt: "20px", color: "red", opacity: "0,60", textAlign: "center" }}>{messageError}</Typography>)}
